...
![](assets/image-20210721185923635.png){data-zoomable}

通过上面的描述，大家应该已经知道了 `数据库管理系统` 和 `数据库` 的关系。那么有有哪些常见的数据库管理系统呢？

### 1.3  常见的数据库管理系统

![](assets/image-20210721184354001.png){data-zoomable}
...
![](assets/image-20210721185303106.png){data-zoomable}
...
![](assets/image-20210404200055449.png){data-zoomable}

选择选择和自己**系统位数**相对应的版本点击右边的`Download`，此时会进到另一个页面，同样在接近页面底部的地方找到如下图所示的位置：

![](https://img2018.cnblogs.com/blog/1556823/201812/1556823-20181220194715840-436169502.png){data-zoomable}

不用理会上面的登录和注册按钮，直接点击 `No thanks, just start my download.` 就可以下载。

![](assets/image-20201109134805641.png){data-zoomable}
...
![](assets/image-20201109134948046.png){data-zoomable}
...
![](assets/1556823-20181220222924783-57600848.png){data-zoomable}
...
![](assets/1556823-20181220223025128-587235464.png){data-zoomable}
...
![](assets/1556823-20181220220242472-524708778.png){data-zoomable}

点击`环境变量`

![](assets/1556823-20181220220359609-736422950.png){data-zoomable}

在`系统变量`中新建MYSQL_HOME

![](assets/image-20201109140222488.png){data-zoomable}

在`系统变量`中找到并**双击**`Path`

![](assets/1556823-20181220220551145-1198958872.png){data-zoomable}

点击`新建`

![](assets/image-20201109135248104.png){data-zoomable}
...
![](assets/image-20201109142704951.png){data-zoomable}

![](assets/image-20201109142737584.png){data-zoomable}
...
![](assets/image-20201109140955772.png){data-zoomable}

tips：如果出现如下错误

![](assets/image-20201109135848054.png){data-zoomable}

是由于权限不足导致的，去`C:\Windows\System32` 下以管理员方式运行 cmd.exe

![](assets/image-20201109140423691.png){data-zoomable}

![](assets/image-20201109140001186.png){data-zoomable}
...
![](assets/image-20201109141325810.png){data-zoomable}
...
![](assets/1556823-20181221093036851-1317238155.png){data-zoomable}
...
![](assets/1556823-20181221093251250-819416425.png){data-zoomable}
...
![](assets/1556823-20181220222422178-61579658.png){data-zoomable}
...
![](assets/image-20210721205130231.png){data-zoomable}
...
* SQL 语句可以单行或多行书写，以分号结尾。
  ...
  ![](assets/image-20210721215223872.png){data-zoomable}
...
* MySQL 数据库的 SQL 语句不区分大小写，关键字建议使用大写。
  ...
  ![](assets/image-20210721215328410.png){data-zoomable}

* 注释

  * 单行注释: -- 注释内容 或 #注释内容(MySQL 特有)

    ![](assets/image-20210721215517293.png){data-zoomable}

    ![](assets/image-20210721215556885.png){data-zoomable}
...
* DDL(Data Definition Language) ： 数据定义语言，用来定义数据库对象：数据库，表，列等
  ...
  ![](assets/image-20210721220032047.png){data-zoomable}

* DML(Data Manipulation Language) 数据操作语言，用来对数据库中表的数据进行增删改

  DML简单理解就对表中数据进行增删改

  ![](assets/image-20210721220132919.png){data-zoomable}
...
![](assets/image-20210721221107014.png){data-zoomable}
...
![](assets/image-20210721223450755.png){data-zoomable}

而在创建数据库的时候，我并不知道db1数据库有没有创建，直接再次创建名为db1的数据库就会出现错误。

![](assets/image-20210721223745490.png){data-zoomable}
...
![](assets/image-20210721224056476.png){data-zoomable}
...
![](assets/image-20210721224435251.png){data-zoomable}
...
![](assets/image-20210721224720841.png){data-zoomable}
...
![](assets/image-20210721230202814.png){data-zoomable}
...
![](assets/image-20210721230332428.png){data-zoomable}
...
![](assets/image-20210721230824097.png){data-zoomable}
...
![](assets/image-20210721231142326.png){data-zoomable}
...
![](assets/image-20210721235108267.png){data-zoomable}
...
![](assets/image-20210721235928346.png){data-zoomable}

第二步：填写连接数据库必要的信息

![](assets/image-20210722000116080.png){data-zoomable}

以上操作没有问题就会出现如下图所示界面：

![](assets/image-20210722000345349.png){data-zoomable}

#### 6.3.2  操作

连接成功后就能看到如下图界面：

![](assets/image-20210722000521997.png){data-zoomable}

* **修改表结构**

通过下图操作修改表结构：

![](assets/image-20210722000740661.png){data-zoomable}

点击了设计表后即出现如下图所示界面，在图中红框中直接修改字段名，类型等信息：

![](assets/image-20210722000929075.png){data-zoomable}

* **编写SQL语句并执行**

按照如下图所示进行操作即可书写SQL语句并执行sql语句。

![](assets/image-20210722001333817.png){data-zoomable}
...
* **练习**
  ...
  * 注意：如果update语句没有加where条件，则会将表中所有数据全部修改！
    ...
    ![](assets/image-20210722204233305.png){data-zoomable}
...
* 查询所有列的数据，列名的列表可以使用*替代
  ...
  ![](assets/image-20210722221534870.png){data-zoomable}
...
* 查询地址信息
  ...
  ![](assets/image-20210722221756380.png){data-zoomable}
...
![](assets/image-20210722190508272.png){data-zoomable}
...
![](assets/image-20210722194410628.png){data-zoomable}
...
![](assets/image-20210722230330366.png){data-zoomable}
...
![](assets/image-20210724104749122.png){data-zoomable}
...
![](assets/image-20210724114548170.png){data-zoomable}
...
![](assets/image-20210724114805350.png){data-zoomable}
...
![](assets/image-20210724115024106.png){data-zoomable}
...
![](assets/image-20210724115149415.png){data-zoomable}
...
![](assets/image-20210724115336916.png){data-zoomable}
...
![](assets/image-20210724115547951.png){data-zoomable}
...
![](assets/image-20210724115826516.png){data-zoomable}
...
![](assets/image-20210724120904180.png){data-zoomable}
...
* 软件的研发步骤
  ...
  ![](assets/image-20210724130925801.png){data-zoomable}
...
* 数据库设计的步骤
  ...
  * 逻辑分析（通过ER图对数据库进行逻辑建模，不需要考虑我们所选用的数据库管理系统）
    ...
    ![](assets/image-20210724131210759.png){data-zoomable}
...
* 表关系
  ...
  * 一对一
    ...
    * 一对一关系多用于表拆分，将一个实体中经常使用的字段放一张表，不经常使用的字段放另一张表，用于提升查询性能
    ...
    ![](assets/image-20210724133015129.png){data-zoomable}
  ...
  * 一对多
    ...
    * 一个部门对应多个员工，一个员工对应一个部门。如下图：
      ...
      ![](assets/image-20210724133443094.png){data-zoomable}
  ...
  * 多对多
    ...
    * 一个商品对应多个订单，一个订单包含多个商品。如下图：
      ...
      ![](assets/image-20210724133704682.png){data-zoomable}
...
* 案例
  ...
  ![](assets/image-20210724134145803.png){data-zoomable}

  经过分析发现，员工表属于多的一方，而部门表属于一的一方，此时我们会在员工表中添加一列（dep_id），指向于部门表的主键（id）：

  ![](assets/image-20210724134318685.png){data-zoomable}
  ...
  ![](assets/image-20210724140456921.png){data-zoomable}
...
* 案例
  ...
  ![](assets/image-20210724134735939.png){data-zoomable}

  经过分析发现，订单表和商品表都属于多的一方，此时需要创建一个中间表，在中间表中添加订单表的外键和商品表的外键指向两张表的主键：

  ![](assets/image-20210724135054834.png){data-zoomable}
  ...
  ![](assets/image-20210724140307910.png){data-zoomable}
...
* 案例
  ...
  ![](assets/image-20210724135346913.png){data-zoomable}

  而在真正使用过程中发现 id、photo、nickname、age、gender 字段比较常用，此时就可以将这张表查分成两张表。

​ ![](assets/image-20210724135649341.png){data-zoomable}
...
![](assets/image-20210724141445785.png){data-zoomable}

### 2.5  数据库设计案例

根据下图设计表及表和表之间的关系：

![](assets/image-20210724141822204.png){data-zoomable}

经过分析，我们分为 `专辑表`  `曲目表`  `短评表`  `用户表`   4张表。

![](assets/image-20210724141550446.png){data-zoomable}
...
![](assets/image-20210724142550839.png){data-zoomable}
...
![](assets/image-20210724173630506.png){data-zoomable}
...
![](assets/image-20210724174212443.png){data-zoomable}

上面语句就是连接查询，那么多表查询都有哪些呢？

* 连接查询

  ![](assets/image-20210724174717647.png){data-zoomable}
...
* 案例
  ...
  * 隐式内连接
    ...
    ![](assets/image-20210724175344508.png){data-zoomable}
  ...
  * 查询 emp的 name， gender，dept表的dname
    ...
    ![](assets/image-20210724175518159.png){data-zoomable}
  ...
  * 显式内连接
    ...
    ![](assets/image-20210724180103531.png){data-zoomable}
...
* 案例
  ...
  * 查询emp表所有数据和对应的部门信息（左外连接）
    ...
    ![](assets/image-20210724180542757.png){data-zoomable}
  ...
  * 查询dept表所有数据和对应的员工信息（右外连接）
    ...
    ![](assets/image-20210724180613494.png){data-zoomable}
...
![](assets/image-20210724224955876.png){data-zoomable}
...
![](assets/image-20210724225537533.png){data-zoomable}
...
* 不加事务演示问题
  ...
  ![](assets/image-20210724230250263.png){data-zoomable}
