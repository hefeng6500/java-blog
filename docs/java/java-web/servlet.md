...
![](assets/1627031023395.png){data-zoomable}
...
等学习完JavaWeb之后，同学们就可以使用Java语言开发我们上述所说的网站。而国内很多大型网站公司也是首选Java语言来解决web互联网相关的问题。那都有哪些公司的系统是使用Java语言的呢?
 ![](assets/20210717183958532.png){data-zoomable}
...
![](assets/1627031933553.png){data-zoomable}
...
![](assets/1627037814180.png){data-zoomable}
...
![](assets/1627038674340.png){data-zoomable}

#### 1.2.4 数据库

* 数据库主要负责存储数据。
* 整个Web的访问过程就如下图所示:
![](assets/1627039320220.png){data-zoomable}
...
![](assets/1627043194238.png){data-zoomable}
...
![](assets/1627046235092.png){data-zoomable}
...
![](assets/1627050004221.png){data-zoomable}
...
* 请求体: POST请求的最后一部分，存储请求参数
   ...
   ![](assets/1627050930378.png){data-zoomable}
...
![](assets/1627278511902.png){data-zoomable}

使用IDEA打开

![](assets/1627278583127.png){data-zoomable}

打开后，可以点击项目中的`html\19-表单验证.html`，使用浏览器打开，通过修改页面中form表单的method属性来测试GET请求和POST请求的参数携带方式。

![](assets/1627278725007.png){data-zoomable}
...
![](assets/1627053710214.png){data-zoomable}
...
![](assets/1627058356051.png){data-zoomable}
...
![](assets/1627060368806.png){data-zoomable}
...
![](assets/1627176045795.png){data-zoomable}
...
![](assets/1627178001030.png){data-zoomable}
...
* 打开`apache-tomcat-8.5.68`目录就能看到如下目录结构，每个目录中包含的内容需要认识下,
  ...
  ![](assets/1627178815892.png){data-zoomable}
...
![](assets/1627179006011.png){data-zoomable}

启动后，通过浏览器访问 `http://localhost:8080`能看到Apache Tomcat的内容就说明Tomcat已经启动成功。

![](assets/1627199957728.png){data-zoomable}

==注意==: 启动的过程中，控制台有中文乱码，需要修改conf/logging.prooperties

![](assets/1627199827589.png){data-zoomable}
...
![](assets/1627200509883.png){data-zoomable}
...
* Tomcat的端口号取值范围是0-65535之间任意未被占用的端口，如果设置的端口号被占用，启动的时候就会包如下的错误
  ...
  ![](assets/1627200780590.png){data-zoomable}

* Tomcat启动的时候，启动窗口一闪而过: 需要检查JAVA_HOME环境变量是否正确配置

![](assets/1627201248802.png){data-zoomable}
...
* Tomcat部署项目： 将项目放置到webapps目录下，即部署完成。
  ...
  * 通过浏览器访问`http://localhost/hello/a.html`，能看到下面的内容就说明项目已经部署成功。
    ...
    ![](assets/1627201572748.png){data-zoomable}
...
* 一般JavaWeb项目会被打包称==war==包，然后将war包放到Webapps目录下，Tomcat会自动解压缩war文件
  ...
  * 通过浏览器访问`http://localhost/haha/a.html`，能看到下面的内容就说明项目已经部署成功。
    ...
    ![](assets/1627201868752.png){data-zoomable}
...
* Maven Web项目结构: 开发中的项目
  ...
  ![](assets/1627202865978.png){data-zoomable}

* 开发完成部署的Web项目

  ![](assets/1627202903750.png){data-zoomable}
...
1. 创建Maven项目
   ...
   ![](assets/1627227574092.png){data-zoomable}

2. 选择使用Web项目骨架

   ![](assets/1627227650406.png){data-zoomable}

3. 输入Maven项目坐标创建项目

   ![](assets/1627228065007.png){data-zoomable}

4. 确认Maven相关的配置信息后，完成项目创建

   ![](assets/1627228413280.png){data-zoomable}

5. 删除pom.xml中多余内容，只留下面的这些内容，注意打包方式 jar和war的区别

   ![](assets/1627228584625.png){data-zoomable}

6. 补齐Maven Web项目缺失的目录结构，默认没有java和resources目录，需要手动完成创建补齐，最终的目录结果如下

   ![](assets/1627228673162.png){data-zoomable}
...
1. 创建Maven项目
   ...
   ![](assets/1627229111549.png){data-zoomable}

2. 选择不使用Web项目骨架

   ![](assets/1627229137316.png){data-zoomable}

3. 输入Maven项目坐标创建项目

   ![](assets/1627229371251.png){data-zoomable}

4. 在pom.xml设置打包方式为war,默认是不写代表打包方式为jar

   ![](assets/1627229428161.png){data-zoomable}

5. 补齐Maven Web项目缺失webapp的目录结构

   ![](assets/1627229584134.png){data-zoomable}

6. 补齐Maven Web项目缺失WEB-INF/web.xml的目录结构

   ![](assets/1627229676800.png){data-zoomable}

7. 补充完后，最终的项目结构如下:

   ![](assets/1627229478030.png){data-zoomable}
...
![](assets/1627204022604.png){data-zoomable}

> 3.掌握不使用骨架的方式创建Maven Web项目

![](assets/1627204076090.png){data-zoomable}
...
1. 打开添加本地Tomcat的面板
   ...
   ![](assets/1627229992900.png){data-zoomable}

2. 指定本地Tomcat的具体路径

   ![](assets/1627230313062.png){data-zoomable}

3. 修改Tomcat的名称，此步骤可以不改，只是让名字看起来更有意义，HTTP port中的端口也可以进行修改，比如把8080改成80

   ![](assets/1627230366658.png){data-zoomable}

4. 将开发项目部署项目到Tomcat中

   ![](assets/1627230913259.png){data-zoomable}
...
5. 部署成功后，就可以启动项目，为了能更好的看到启动的效果，可以在webapp目录下添加a.html页面
   ...
   ![](assets/1627233265351.png){data-zoomable}

6. 启动成功后，可以通过浏览器进行访问测试

   ![](assets/1627232743706.png){data-zoomable}

7. 最终的注意事项

   ![](assets/1627232916624.png){data-zoomable}

至此，IDEA中集成本地Tomcat进行项目部署的内容我们就介绍完了，整体步骤如下，大家需要按照流程进行部署操作练习。

![](assets/1627205657117.png){data-zoomable}
...
![](assets/1627233963315.png){data-zoomable}

==注意:==

* 如果选中项目并右键点击后，看不到Run Maven和Debug Maven，这个时候就需要在IDEA中下载Maven Helper插件，具体的操作方式为: File --> Settings --> Plugins --> Maven Helper ---> Install,安装完后按照提示重启IDEA，就可以看到了。

![](assets/1627234184076.png){data-zoomable}
...
![](assets/1627234763207.png){data-zoomable}
...
* Servlet是JavaEE规范之一，其实就是一个接口，将来我们需要定义Servlet类实现Servlet接口，并由web服务器运行Servlet
  ...
  ![](assets/1627234972853.png){data-zoomable}
...
![](assets/1627236923139.png){data-zoomable}
...
* 通过案例演示下上述的生命周期
  ...
  */
  ...
  ![](assets/1627239292226.png){data-zoomable}
...
![](assets/1627240593506.png){data-zoomable}
...
* 一个Servlet,可以配置多个urlPattern
  ...
  ![](assets/1627272805178.png){data-zoomable}
...
* ==urlPattern配置规则==
  ...
  * 精确匹配
    ...
    ![](assets/1627273174144.png){data-zoomable}
  ...
  * 目录匹配
    ...
    ![](assets/1627273184095.png){data-zoomable}
  ...
  * 扩展名匹配
    ...
    ![](assets/1627273194118.png){data-zoomable}
    ...
    ![](assets/1627274483755.png){data-zoomable}

    2. 如果路径配置的是`*.do`,那么在*.do的前面不能加`/`,否则会报错

    ![](assets/1627274368245.png){data-zoomable}

  * 任意匹配

    ![](assets/1627273201370.png){data-zoomable}
